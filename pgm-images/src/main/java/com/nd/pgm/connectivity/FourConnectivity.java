...
public class FourConnectivity implements IConnectivity {
    ...
    public int compute(int[][] imageData, int i, int j, int vSize, int hSize, IOperator operator) {
        ...
        for (int ii = vStart; ii <= vEnd; ii++) {
            try {
                outPixel = computeOperator(operator, outPixel, imageData[ii][j], i, j, ii, j);
        ...
        for (int jj = hStart; jj <= hEnd; jj++) {
            if (jj == j)
                continue;
            try {
                outPixel = computeOperator(operator, outPixel, imageData[i][jj], i, j, i, jj);
            } catch (IndexOutOfBoundsException ex) {
                assert false : "No deberia saltar " + ex;
            }
        }
